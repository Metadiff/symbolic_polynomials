...
pub fn mul_test() {
    ...
    assert!(abc_times_2.powers ==
        vec![(Composite::Variable(0), 1),
        (Composite::Variable(1), 1),
        (Composite::Variable(2), 1)]);
...
pub fn div_test() {
    ...
    assert!(bc2.powers ==
        vec![(Composite::Variable(1), 1), (Composite::Variable(2), 1)]);

    // 2ac
    let ac2 = &abc_times_2 / &b;
    assert!(ac2.coefficient == 2);
    assert!(ac2.powers ==
        vec![(Composite::Variable(0), 1), (Composite::Variable(2), 1)]);
    ...
    assert!(c2.powers == vec![(Composite::Variable(2), 1)]);

    // 2b
    let b2 = &abc_times_2 / &ac;
    assert!(b2.coefficient == 2);
    assert!(b2.powers == vec![(Composite::Variable(1), 1)]);

    // 2a
    let a2 = &abc_times_2 / &bc;
    assert!(a2.coefficient == 2);
    assert!(a2.powers == vec![(Composite::Variable(0), 1)]);
